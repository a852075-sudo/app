import { ROOMS, operationalRooms, roomById, loadSettings, saveSettings } from "./setting.js";
import {
  calculateAlert,
  calculateDailyRows,
  calculateSummary,
...
export function createUI(state, actions) {
  ...
  function render() {
    ...
    subtitle.textContent = room ? `${state.year} 年 ${String(state.month).padStart(2, "0")} 月` : "智慧試算、Google Sheets 同步與離線盤點";
  ...
  function renderHome(target) {
    target.append(el("section", "hero-panel", `
      <div>
        <h2>今日盤點工作台</h2>
        <p>手機版 PWA，可離線記錄，恢復網路後再同步 Google Sheets。</p>
      </div>
      <span class="status-pill"><span class="material-symbols-rounded">offline_bolt</span>離線可用</span>
    `));
...
function renderRoom(target, room) {
  ...
  function renderReferenceRoomTable(room, config) {
    ...
    `;
    const body = table.querySelector("tbody");
    const balances = Object.fromEntries(config.groups.map((group, index) => [
      group.key,
      group.columns.includes("balance") && index === 0 ? Number(rows[0]?.openingStock || 0) : 0
    ]));
    rows.forEach((row) => {
      const tr = document.createElement("tr");
      const cells = config.groups.flatMap((group) => {
        const totals = itemTotals(row.entries, ...group.aliases);
        balances[group.key] = (balances[group.key] || 0) + totals.inQty - totals.usedQty - totals.wasteQty;
        return group.columns.map((column) => renderReferenceCell(column, totals, balances[group.key]));
      });
      tr.innerHTML = `
  ...
  function renderSyringeReferencePanel(room) {
    ...
    const panel = el("section", "syringe-panel", `
      <div class="table-toolbar">
        ...
        <button class="tonal-btn" type="button" data-add><span class="material-symbols-rounded">add</span>新增當日登記</button>
      </div>
    ...
    `);
    panel.querySelector("[data-add]").addEventListener("click", () => openEntryDialog(room, isoDate(state.year, state.month, new Date().getDate())));
    return panel;
  }
  ...
  function openSettings() {
    ...
    document.querySelector("#initialStockInputs").innerHTML = operationalRooms().map((room) => `
      <div class="settings-row">
        <strong>${room.name}</strong>
        <input name="stock:${room.id}" type="number" min="0" step="1" value="${settings.initialStocks[room.id] || 0}">
      </div>
    `).join("");
  ...
  async function syncCurrentRoom() {
    ...
    try {
      await fetchRoomEntries(state.roomId);
      render();
      toast("已同步 Google Sheets");
    } catch (error) {
      toast(error.message);
    }
  }
...
function renderReferenceCell(column, totals, balance) {
  const value = {
    people: totals.usedQty,
    in: totals.inQty,
    balance,
    used: totals.usedQty,
    dose: totals.usedQty,
...
function buildContrastSummaryRows(year, month) {
  const roomIds = operationalRooms().filter((room) => room.type !== "syringe").map((room) => room.id);
  const monthEntries = roomIds.flatMap((roomId) => entriesFor(roomId, year, month));
  return CONTRAST_SUMMARY_ITEMS.map((definition) => {
    const matched = filterEntriesByAliases(monthEntries, definition.aliases);
    const inQty = sumEntries(matched, "inQty");
    const usedQty = sumEntries(matched, "usedQty");
    const wasteQty = sumEntries(matched, "wasteQty");
    return {
      name: definition.name,
      people: usedQty,
      consumed: usedQty,
      roomEnding: Math.max(0, inQty - usedQty - wasteQty),
      requestQty: 0,
      cabinetStock: 0
    };
  });
