...
async function saveSyringeReport(metrics, year, monthValue) {
  ...
  const rows = [
    ...
    { recordedAt: new Date().toISOString(), month, room: "高壓針筒全院總計", usedQty: metrics.total }
  ];
  try {
    await appendReportRows({ sheet: settings.sheets.syringe || "Sheet J", headers, rows });
    toast(`已追加儲存 ${month} 高壓針筒月紀錄`);
  } catch (error) {
    toast(error.message);
  }
}
...
const REFERENCE_TABLES = {
  ct1: {
    groups: [
      ...
      referenceGroup("injector", "注射器", ["注射器"], ["dose"]),
      referenceGroup("syringeRefill", "針筒補充", ["針筒補充"], ["refill"])
    ]
  },
  ct3: {
    groups: [
